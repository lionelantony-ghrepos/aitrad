...
test.describe("P0 portfolio P&L @P0", () => {
  test.describe.configure({ timeout: 60_000 });
  test.beforeEach(async ({ page, request }) => {
    await request.post("/api/e2e/reset");
    await signUpThroughWizard(
      page,
      `p0-port-${Date.now()}-${Math.random().toString(16).slice(2)}@example.com`,
    );
  });

  test("fill buy then +5% tick revalues unrealized @TC-018-02 @P0", async ({ page }) => {
