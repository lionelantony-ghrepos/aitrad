...
await build({
  absWorkingDir: root,
  entryPoints: [path.join(root, "insforge/functions/monitor-runner-src.ts")],
  outfile: path.join(root, "insforge/functions/monitor-runner.ts"),
  bundle: true,
  format: "esm",
  platform: "neutral",
  target: "esnext",
  banner: { js: "// bundled from insforge/functions/monitor-runner-src.ts\n" },
  external: ["npm:@insforge/sdk"],
});

const outfile = path.join(root, "insforge/functions/monitor-runner.ts");
