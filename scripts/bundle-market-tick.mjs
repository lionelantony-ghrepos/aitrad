...
await build({
  absWorkingDir: root,
  entryPoints: [path.join(root, "insforge/functions/market-tick-src.ts")],
  outfile: path.join(root, "insforge/functions/market-tick.ts"),
  bundle: true,
  format: "esm",
  platform: "neutral",
  target: "esnext",
  banner: { js: "// insforge/functions/market-tick-src.ts\n" },
  external: ["npm:@insforge/sdk"],
});

const outfile = path.join(root, "insforge/functions/market-tick.ts");
