...
await build({
  absWorkingDir: root,
  entryPoints: [path.join(root, "insforge/functions/health-service-src.ts")],
  outfile: path.join(root, "insforge/functions/health-service.ts"),
  bundle: true,
  format: "esm",
  platform: "neutral",
  target: "esnext",
  banner: { js: "// bundled from insforge/functions/health-service-src.ts\n" },
  external: ["npm:@insforge/sdk"],
});

const outfile = path.join(root, "insforge/functions/health-service.ts");
