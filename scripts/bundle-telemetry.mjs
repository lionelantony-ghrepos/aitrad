...
await build({
  absWorkingDir: root,
  entryPoints: [path.join(root, "insforge/functions/telemetry-src.ts")],
  outfile: path.join(root, "insforge/functions/telemetry.ts"),
  bundle: true,
  format: "esm",
  platform: "neutral",
  target: "esnext",
  banner: { js: "// bundled from insforge/functions/telemetry-src.ts\n" },
  external: ["npm:@insforge/sdk"],
});

const outfile = path.join(root, "insforge/functions/telemetry.ts");
