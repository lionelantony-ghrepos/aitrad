...
await build({
  absWorkingDir: root,
  entryPoints: [path.join(root, "insforge/functions/admin-users-src.ts")],
  outfile: path.join(root, "insforge/functions/admin-users.ts"),
  bundle: true,
  format: "esm",
  platform: "neutral",
  target: "esnext",
  banner: { js: "// bundled from insforge/functions/admin-users-src.ts\n" },
  external: ["npm:@insforge/sdk"],
});

const outfile = path.join(root, "insforge/functions/admin-users.ts");
