...
It can also **act**: "add NVDA to my watchlist", "alert me if AAPL drops below 200", "buy 10 MSFT at market". Safe actions run instantly; **orders always come back as an approval card** in the Copilot thread (and the status-bar notification center) with **Approve** / **Reject** — nothing trades without your explicit click, and approved orders still pass every risk rule. Open the Copilot **Monitors** tab to list standing instructions, pause/resume or delete them, reset throttle so the next qualifying event can alert again, read a plain-English explanation of the compiled condition, and see trigger history. Ask Copilot to create one ("tell me if any position drops 5% in a day"); when it fires you get an alert with a two-sentence explanation citing the data. Open the **Briefs** tab (or **Generate brief** on Description / Portfolio) for a Morning Brief, Instrument Brief, or Portfolio Health report. Citation chips open News or DES. **Export PDF** downloads a server-rendered copy. Tick **Morning brief at simulated open** if you want the digest written automatically when the paper session opens.
