...
// bundled from insforge/functions/rules-service-src.ts
...
async function evaluateDomain(domain, context, ports, options = {}) {
...
function mergeDomainResults(domain, evaluations) {
...
async function handleRulesServiceRequest(input) {
  ...
  const parsed = evaluateDomainRequestSchema.safeParse({
    ...raw,
    op: raw.op === void 0 ? void 0 : "evaluateDomain",
  });
  if (!parsed.success) {
...
var cache = new PublishedRulesCache();
...
async function loadPublishedTables(db, domain) {
  ...
  const tableIds = (bindings ?? []).map((row) => row.table_id);
  ...
  const published = (tables ?? []).filter((row) => wanted.has(row.id));
  ...
  return published.map((table) => {
    const rec = table;
    const tableRows = rowsByTable.get(rec.id) ?? [];
    return {
      domain,
...
var rules_service_src_default = withFunctionLog("rules-service", async function (req) {
  if (req.method === "OPTIONS") {
...
module.exports = rules_service_src_default;
