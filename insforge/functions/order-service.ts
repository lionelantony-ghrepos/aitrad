...
// bundled from insforge/functions/order-service-src.ts
...
function buildOrderFacts(input) {
  const { draft } = input;
...
function applyFeeSchedule(feeOutcome, input) {
  ...
  const dataFee = asNumber(feeOutcome.data_fee_monthly) ?? void 0;
  return {
    commission_usd: commission,
    sec_fee: secFee,
    taf,
    ...(dataFee !== void 0 ? { data_fee_monthly: dataFee } : {}),
  };
}
function asRecords(outcome) {
  if (Array.isArray(outcome)) {
    return outcome.filter((row) => typeof row === "object" && row !== null);
  }
...
function reasonFromOutcome(row) {
...
async function reserveBuyingPower(admin, accountId, userId, amount) {
...
async function releaseBuyingPower2(admin, accountId, userId, amount) {
...
async function publishOrder(admin, userId, order) {
...
function requireAdminWriter(baseUrl) {
  ...
  return createAdminClient2({
    baseUrl,
    apiKey,
  });
}
var order_service_src_default = withFunctionLog("order-service", async function (req) {
  if (req.method === "OPTIONS") {
  ...
  try {
    if (op === "preview") {
      const parsed2 = orderPreviewRequestSchema.parse({
        ...body,
        op: "preview",
      });
    ...
    if (op === "cancel") {
      ...
      if (held > 0) {
        await releaseBuyingPower2(admin2, current.account_id, userId, held);
      }
...
module.exports = order_service_src_default;
