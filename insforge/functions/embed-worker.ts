...
// packages/rag/src/hybrid.ts
function newsEmbedText(item) {
...
function nextEmbedOutcome(input) {
...
async function runEmbedCycle(items, deps) {
  let embedded = 0;
  let retried = 0;
  let deadLettered = 0;
  for (const item of items) {
...
var embed_worker_src_default = withFunctionLog("embed-worker", async function (req) {
  if (req.method !== "POST") {
  ...
  const pending = asRows(pendingRpc.data).map((row) => {
