...
// bundled from insforge/functions/audit-service-src.ts
...
// insforge/functions/audit-service-src.ts
...
// packages/rules-engine/src/audit-chain.ts
...
// packages/rules-engine/src/audit-admin.ts
function actionForOp(op) {
  if (op === "setRetention") {
    return "audit:write";
  }
  return "audit:read";
}
async function handleAuditServiceRequest(input) {
  ...
  if (op === "cron") {
    ...
    if (!parsed.data.force) {
      ...
      if (last === day) {
        const chain3 = await input.ports.loadChainStatus();
        return {
          status: 200,
          body: { verified: chain3.ok, chain: chain3, purged: 0, alerted: 0, skipped: true },
        };
      }
    }
    const chain2 = await input.ports.verifyChain();
    await input.ports.saveChainStatus(chain2);
    let alerted = 0;
    if (!chain2.ok) {
      ...
      for (const adminId of admins) {
        await input.ports.writeAuditLog({
          ...
          entity_id: chain2.broken_id,
          payload: {
            reason: chain2.reason,
            expected_hash: chain2.expected_hash,
            actual_hash: chain2.actual_hash,
          },
        });
        alerted += 1;
      }
    ...
    await input.ports.writeAuditLog({
      ...
      action: "audit:cron",
      entity_type: "audit_log",
      payload: { verified: chain2.ok, purged, alerted },
    });
    return {
      status: 200,
      body: { verified: chain2.ok, chain: chain2, purged, alerted, skipped: false },
    };
  }
  if (input.isService) {
  ...
  if (op === "verify") {
    const chain2 = await input.ports.verifyChain(parsed.data.from, parsed.data.to);
    return { status: 200, body: chain2 };
  }
  if (op === "export") {
  ...
  if (op === "getConfig") {
    const [retention_days, chain2] = await Promise.all([
      input.ports.getRetentionDays(),
      input.ports.loadChainStatus(),
    ]);
    return { status: 200, body: { retention_days, chain: chain2, can_write: canWrite } };
  }
...
var RETENTION_KEY = "audit.retention_days";
var CHAIN_KEY = "audit.chain_status";
var CRON_KEY = "audit.last_cron_date";
...
async function readFlag(db, key) {
...
var audit_service_src_default = withFunctionLog("audit-service", async function (req) {
  if (req.method === "OPTIONS") {
  ...
  const result = await handleAuditServiceRequest({
    ...
    ports: {
      ...
      async listAudit(filter) {
        ...
        const rows = asRows2(listed.data).map((row) => auditLogSchema.parse(row));
      ...
      async verifyChain(from, to) {
        ...
        return {
          ...
          broken_id: row.broken_id ?? null,
          expected_hash: row.expected_hash ?? null,
          actual_hash: row.actual_hash ?? null,
      ...
      async listAdminUserIds() {
        ...
        return asRows2(listed.data).map((row) => row.user_id);
      },
...
module.exports = audit_service_src_default;
