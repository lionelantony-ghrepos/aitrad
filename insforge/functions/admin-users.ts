...
// bundled from insforge/functions/admin-users-src.ts
...
// insforge/functions/admin-users-src.ts
...
// packages/rules-engine/src/admin-users.ts
async function handleAdminUsersRequest(input) {
  ...
  return {
    status: 200,
    body: { ok: true, user_id: parsed.data.user_id, role: parsed.data.role },
  };
}
...
var admin_users_src_default = withFunctionLog("admin-users", async function (req) {
  if (req.method === "OPTIONS") {
      ...
      async listUsers() {
        ...
        return rows.map((row) =>
          adminUserRowSchema.parse({
            ...
            role: userRoleSchema.parse(row.role ?? "trader"),
          }),
        );
      },
...
module.exports = admin_users_src_default;
