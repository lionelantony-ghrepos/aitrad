...
// bundled from insforge/functions/health-service-src.ts
...
// insforge/functions/health-service-src.ts
...
function entitlementHttpStatus(result) {
...
// packages/rules-engine/src/telemetry.ts
async function handleHealthServiceRequest(input) {
  ...
  const result = await handleHealthServiceRequest({
    ...
    ports: {
      ...
      async listTelemetry() {
        ...
        return asRows2(data).map((row) => telemetryRecordSchema.parse(row));
      },
...
var health_service_src_default = withFunctionLog("health-service", handle);

module.exports = health_service_src_default;
